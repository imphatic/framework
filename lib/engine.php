...
class frameworkEngine extends frameworkError implements Iterator
{
    ...
    */
    ...
    function paging($page = NULL, $displayFirstLast = FALSE, $displayPageNums = FALSE, $maxPageNumbers=NULL)
    {
        ...
        if($displayFirstLast)
        {
            $return .= "<a href='" . $this->firstPageHREF . "' class='first_page'>First</a>&nbsp;&nbsp;";
        }

        if (!$this->firstPage)
        {
            $return .= '<a href="' .  sprintf("%s?" . $this->objUniqueKey . $this->pageVar . "=%d%s", $gotoPage, max(0, $this->curPage - 1), $this->queryString) . '">&lt; Previous</a> &nbsp;&nbsp;';
        }
            ...
            if(count($this->pageHREF) > 0)
            {
                ...
                foreach($this->pageHREF as $pageNum => $href)
                {
                    ...
                    if($maxPageNumbers > 0 && $this->totalPages > $maxPageNumbers)
                    {
                        ...
                        {
                            $return .= '<a href="' . $href . '" class="page_num'.$active.'">' . ($pageNum) . '</a>&nbsp;';
                        } else {
        ...
        if (!$this->lastPage)
        {
            $return .= '&nbsp;&nbsp;<a href="' . sprintf("%s?" . $this->objUniqueKey . $this->pageVar . "=%d%s", $gotoPage, min($this->totalPages, $this->curPage + 1), $this->queryString) . '">Next &gt;</a>';
        }

        if($displayFirstLast)
        {
            $return .= "&nbsp;&nbsp;<a href='" . $this->lastPageHREF . "' class='last_page'>Last</a>";
        }
        return $return;
    }
